...
func (ctx *lsh512ContextGo) Size() int {
	return ctx.outlenbytes
}

func (ctx *lsh512ContextGo) BlockSize() int {
	return BlockSize
}

func (ctx *lsh512ContextGo) Reset() {
	internal.MemclrU64(ctx.tcv[:])
	internal.MemclrU64(ctx.msg[:])
...
func (ctx *lsh512ContextGo) Write(p []byte) (n int, err error) {
	if len(p) == 0 {
		return
	}
	plen := len(p)
...
func (ctx *lsh512ContextGo) Sum(p []byte) []byte {
	b0 := *ctx
	hs := b0.checkSum()
	return append(p, hs[:ctx.Size()]...)
}

func (ctx *lsh512ContextGo) checkSum() [Size]byte {
...
func (ctx *lsh512ContextGo) compress(data []byte) {
	ctx.msgExpansion(data)

	for i := 0; i < numStep/2; i++ {
		ctx.step(2*i, alphaEven, betaEven)
...
func (ctx *lsh512ContextGo) msgExpansion(in []byte) {
	for i := 0; i < 32; i++ {
		ctx.msg[i] = binary.LittleEndian.Uint64(in[i*8:])
	}

	for i := 2; i <= numStep; i++ {
		idx := 16 * i
		ctx.msg[idx+0] = ctx.msg[idx-16] + ctx.msg[idx-29]
...
func (ctx *lsh512ContextGo) step(stepidx, alpha, beta int) {
	var vl, vr uint64
	...
	for colidx := 0; colidx < 8; colidx++ {
		...
		ctx.tcv[colidx+8] = bits.RotateLeft64(vr, gamma[colidx])
	}
